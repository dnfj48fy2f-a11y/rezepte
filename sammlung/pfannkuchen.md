---
titel: Pfannkuchen
title_en: German Pancakes
kategorie: fruehstueck
tags: [pfannkuchen, kinder, grundrezept]
portionen: 4
zeit_aktiv: 30 min
zeit_gesamt: 30 min
quelle: "Eigene Sammlung (Word: Breakfast/Pfannkuchen.doc)"
sprache_original: de
hinzugefuegt: 2026-07-14
bewertung:
foto: fotos/pfannkuchen.jpg
---

## Zutaten
- 350 g Mehl
- 80 g Zucker
- 3 Eier
- 0,5 l Milch

## Zubereitung
1. Alle Zutaten zu einem glatten Teig verrühren.
2. Portionsweise in der Pfanne ausbacken.
3. Für Apfelpfannkuchen: Zimt in den Teig geben und Apfelscheiben in Zimt einlegen. Zuerst die Äpfel in die Pfanne geben, dann den Teig darüber gießen.

## Notizen
Eizahl im Original nicht angegeben (geschätzt: 3), Portionszahl geschätzt. Es gab eine identische Dropbox-Konfliktkopie („conflicted copy 2021-02-22"), die zusammengeführt wurde. Foto: oetker.de (Pfannkuchen/Eierkuchen, klassische dünne Variante).

## English
### Ingredients
- 350 g flour
- 80 g sugar
- 3 eggs
- 0.5 l milk

### Instructions
1. Mix all ingredients into a smooth batter.
2. Fry in batches in a pan.
3. For apple pancakes: add cinnamon to the batter and toss apple slices in cinnamon. Put the apples into the pan first, then pour the batter over them.

### Notes
Number of eggs not given in the original (estimated: 3); serving count estimated. An identical Dropbox conflicted copy was merged into this file.
